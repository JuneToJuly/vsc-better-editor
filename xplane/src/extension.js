...
function activate(context) {
  ...
  const replaySession = new ReplaySession(async (repository, commit) => {
    const item = await timelineProvider.findItem(repository.root, commit.hash);
    if (!item) {
      return;
    }
    ...
    try {
      ...
      // The timeline view may be hidden or rebuilding. Diff navigation should
      // still succeed even when VS Code cannot reveal the tree item yet.
    }
  });
