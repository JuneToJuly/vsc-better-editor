...
class ReplayTimelineProvider {
  ...
  render(){
    ...
    const payload=JSON.stringify({events,position:session.position,title:session.result?.displayName||'Test'}).replace(/</g,'\\u003c');
    this.view.webview.html=`<!doctype html><html><head><meta charset="UTF-8"><style>
      *{box-sizing:border-box}html,body{height:100%;margin:0}body{font-family:var(--vscode-font-family);color:var(--vscode-foreground);background:var(--vscode-panel-background,var(--vscode-editor-background));overflow:hidden}.app{height:100%;display:grid;grid-template-rows:auto 1fr}.bar{display:flex;gap:6px;align-items:center;padding:6px 8px;border-bottom:1px solid var(--vscode-panel-border)}input{flex:1;min-width:120px;background:var(--vscode-input-background);color:var(--vscode-input-foreground);border:1px solid var(--vscode-input-border,var(--vscode-panel-border));padding:5px 7px}.status{font-size:11px;color:var(--vscode-descriptionForeground);white-space:nowrap}.rows{overflow:auto;font-family:var(--vscode-editor-font-family);font-size:12px}.row{width:100%;display:grid;grid-template-columns:62px minmax(170px,1fr) 150px minmax(220px,2fr);gap:8px;padding:5px 8px;border:0;border-bottom:1px solid color-mix(in srgb,var(--vscode-panel-border) 55%,transparent);background:transparent;color:inherit;text-align:left;cursor:pointer}.row:hover{background:var(--vscode-list-hoverBackground)}.row.active{background:var(--vscode-list-activeSelectionBackground);color:var(--vscode-list-activeSelectionForeground)}.seq,.loc{color:var(--vscode-descriptionForeground)}.row.active .seq,.row.active .loc{color:inherit;opacity:.8}.code{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.hidden{display:none}</style></head><body><div class="app"><div class="bar"><input id="search" placeholder="Search replay — method, file, line, source…"><span id="status" class="status"></span></div><div id="rows" class="rows"></div></div><script>
      const vscode=acquireVsCodeApi(),model=${payload},root=document.getElementById('rows'),search=document.getElementById('search'),status=document.getElementById('status');let query='';
      const esc=s=>String(s??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;'}[c]));
      function render(){root.innerHTML='';let shown=0;for(const e of model.events){const hay=[e.sequence,e.method,e.file,e.line,e.text,e.file+':'+e.line].join(' ').toLowerCase();if(query&&!hay.includes(query))continue;shown++;const b=document.createElement('button');b.className='row '+(e.index===model.position?'active':'');b.innerHTML='<span class="seq">#'+esc(e.sequence)+'</span><b>'+esc(e.method)+'</b><span class="loc">'+esc(e.file)+':'+e.line+'</span><span class="code">'+esc(e.text)+'</span>';b.onclick=()=>vscode.postMessage({command:'seek',index:e.index});root.appendChild(b)}status.textContent=(model.position+1)+' / '+model.events.length+(query?' · '+shown+' matches':'');requestAnimationFrame(()=>root.querySelector('.active')?.scrollIntoView({block:'center'}))}
      search.oninput=()=>{query=search.value.trim().toLowerCase();render()};render();</script></body></html>`;
  }
}
