...
function activate(context) {
    ...
    const disposable = vscode.commands.registerCommand('windowSearch.run', async function () {
        ...
        // 4. Scan files
        const results = [];
        const CONCURRENCY = 6; // 🔥 tune this (4–10 is ideal)

        for (let i = 0; i < files.length; i += CONCURRENCY) {
            const batch = files.slice(i, i + CONCURRENCY);

            const batchResults = await Promise.all(
                batch.map(file => processfile(file, terms))
            );
...
async function processfile(file, terms) {
    try {
        ...
        for (let i = 0; i < doc.lineCount; i++) {
            ...
            lines.push(text);
            lowerLines.push(text.toLowerCase());
        }

        const lowerTerms = terms.map(t => t.toLowerCase());

        // 🔥 pick best filter term (shortest = usually rarest)
        const firstTerm = lowerTerms.reduce((a, b) =>
        ...
        for (let i = 0; i <= lines.length - windowSize;) {
            ...
            for (let j = 0; j < windowSize && (i + j) < lines.length; j++) {
                if (lowerLines[i + j].includes(firstTerm)) {
                    foundFirst = true;
                    break;
                }
            }
            ...
            // 🔥 track which terms we’ve seen
            const found = new Array(lowerTerms.length).fill(false);

            for (let j = 0; j < windowSize && (i + j) < lines.length; j++) {
                const lineLower = lowerLines[i + j];

                for (let t = 0; t < lowerTerms.length; t++) {
                    if (!found[t] && lineLower.includes(lowerTerms[t])) {
                        found[t] = true;
                    }
                }
            }
            ...
            const matchesAll = found.every(v => v);
