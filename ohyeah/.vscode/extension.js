...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .toolbar { position: sticky; top: 0; z-index: 20; display: grid; grid-template-columns: minmax(0, 1fr) auto auto; align-items: center; gap: 14px; padding: 10px 14px; background: var(--vscode-editor-background); border-bottom: 1px solid var(--vscode-panel-border); }
  ...
  .cursor-position-row { display: grid; grid-template-columns: 34px minmax(0, 1fr); gap: 9px; min-width: 0; height: 7px; margin-top: -1px; opacity: .95; }
  .cursor-position-track { min-width: 0; padding-left: 10px; overflow: hidden; white-space: pre; font-family: var(--vscode-editor-font-family); }
  .cursor-position-caret { color: var(--vscode-focusBorder); font-weight: 800; line-height: 7px; }
  ...
  function renderCodeContext(location, targetLine, targetCharacter) {
    ...
    const rendered = lines.map((text, index) => {
      ...
      if (isTarget) {
        ...
        displayed = highlightCode(text.slice(0, safeCharacter), location.fileName) +
          '<span class="cursor-inline" title="Last cursor position: column ' + (safeCharacter + 1) + '" aria-label="Last cursor position at column ' + (safeCharacter + 1) + '"></span>' +
          highlightCode(text.slice(safeCharacter), location.fileName);
      } else {
