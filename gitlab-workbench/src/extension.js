...
const {GlabClient,parseProjectUrl,sameProjectUrl}=require('./services/glabClient');
...
function activate(context){
 ...
 // views after an extension-host reload; a real review will set this true again.
 void vscode.commands.executeCommand('setContext','gitlabWorkbench.reviewActive',false);
 void vscode.commands.executeCommand('setContext','gitlabWorkbench.hideResolvedComments',false);
 ...
 cmd('gitlabWorkbench.addProject',async()=>{
  ...
  await c.update('managedProjects',[...current,value.trim()],vscode.ConfigurationTarget.Global);tree.refresh();await mrWebview?.refresh();issueTree.refresh();vscode.window.showInformationMessage(`Added GitLab project: ${parsed.host}/${parsed.project} (${parsed.protocol||'remote'})`);
 });
 ...
 cmd('gitlabWorkbench.openCommitDashboard',async()=>openCommitDashboard(client));
 cmd('gitlabWorkbench.openMr',mr=>openMr(client,mr));
 cmd('gitlabWorkbench.openWatcherEvent',async e=>{
  if(!e?.mr)return;
  const mr=e.mr;
  const hasDiscussion=!!e.discussionId;
  const positioned=hasDiscussion&&!!e.path;
  const tab=hasDiscussion?(positioned?'feedback':'conversation'):'conversation';
  client().output?.appendLine(`[watchers] open !${mr.iid} (${mr.repoName||mr.repo||mr.project||''}) in Workbench${hasDiscussion?` → ${tab}`:''} — no watcher refresh`);
  await openMr(client,mr,hasDiscussion?{tab,discussionId:String(e.discussionId)}:{});
 });
 ...
 cmd('gitlabWorkbench.toggleReviewWhitespace',async()=>{review.ignoreWhitespace=!review.ignoreWhitespace;if(review.mr)await showReviewFile();vscode.window.setStatusBarMessage(`Review diff: whitespace ${review.ignoreWhitespace?'ignored':'shown'}`,2500);});
...
async function openCommitDashboard(client){
 const p=vscode.window.createWebviewPanel('gitlabWorkbenchCommits','Commits — GitLab Workbench',vscode.ViewColumn.One,{enableScripts:true,retainContextWhenHidden:true});
 let commits=[],me='',branchScope='default';
 const shell=(body)=>`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><style>
 body{font-family:var(--vscode-font-family);color:var(--vscode-foreground);background:var(--vscode-editor-background);padding:22px 28px;margin:0}*{box-sizing:border-box}.top{display:flex;justify-content:space-between;align-items:flex-start;gap:20px;margin-bottom:18px}h1{font-size:26px;margin:0 0 5px}.sub,.muted{color:var(--vscode-descriptionForeground)}button,select,input{font:inherit;color:inherit;background:var(--vscode-input-background);border:1px solid var(--vscode-input-border,var(--vscode-panel-border));border-radius:5px;padding:7px 10px}.filters{display:grid;grid-template-columns:minmax(220px,1fr) 165px 180px 180px auto auto;gap:8px;position:sticky;top:0;background:var(--vscode-editor-background);padding:8px 0 14px;z-index:3}.day{margin:20px 0 6px;font-size:12px;font-weight:700;letter-spacing:.08em;color:var(--vscode-descriptionForeground);text-transform:uppercase}.head,.row{display:grid;grid-template-columns:112px minmax(160px,230px) 88px 125px minmax(320px,1fr);gap:12px;align-items:center}.head{font-size:11px;font-weight:700;color:var(--vscode-descriptionForeground);padding:7px 10px;border-bottom:1px solid var(--vscode-panel-border)}.row{padding:9px 10px;border-bottom:1px solid var(--vscode-panel-border);cursor:pointer;min-height:42px}.row:hover{background:var(--vscode-list-hoverBackground)}.time{font-variant-numeric:tabular-nums;font-weight:600;display:flex;flex-direction:column;gap:1px}.time small{font-weight:400;color:var(--vscode-descriptionForeground);font-size:10px}.repo{font-weight:600;overflow:hidden;text-overflow:ellipsis}.repo-badge{display:inline-block;max-width:100%;padding:3px 7px;border-radius:999px;background:var(--vscode-badge-background);color:var(--vscode-badge-foreground);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.mine.active{background:var(--vscode-button-background);color:var(--vscode-button-foreground);border-color:var(--vscode-button-background)}.sha{font-family:var(--vscode-editor-font-family);color:var(--vscode-textLink-foreground)}.title{white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:flex;flex-direction:column;gap:3px}.commit-context{font-size:10px;color:var(--vscode-descriptionForeground);overflow:hidden;text-overflow:ellipsis}.commit-context code{font-family:var(--vscode-editor-font-family)}.commit-context .mr-pending{opacity:.65}.commit-context .mr{color:var(--vscode-textLink-foreground);font-weight:600;background:none;border:0;padding:0 2px;cursor:pointer;font-size:10px}.commit-context .mr:hover{text-decoration:underline}.detail{display:none;padding:12px 16px 16px 115px;border-bottom:1px solid var(--vscode-panel-border);background:var(--vscode-sideBar-background)}.detail.open{display:block}.commit-mrs{display:flex;align-items:center;gap:8px;margin-bottom:12px;min-height:24px}.detail-mr{font-size:12px!important}.stats{display:flex;gap:18px;margin-bottom:10px;font-weight:600}.files{display:grid;grid-template-columns:minmax(260px,1fr) auto;gap:5px 16px;max-width:850px}.file{font-family:var(--vscode-editor-font-family);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.plus{color:var(--vscode-gitDecoration-addedResourceForeground)}.minus{color:var(--vscode-gitDecoration-deletedResourceForeground)}.empty{padding:50px;text-align:center;color:var(--vscode-descriptionForeground)}@media(max-width:850px){.head,.row{grid-template-columns:90px 150px 80px 1fr}.author{display:none}.filters{grid-template-columns:1fr 1fr}.detail{padding-left:16px}}</style></head><body>${body}</body></html>`;
 const loading=()=>{p.webview.html=shell('<div class="top"><div><h1>Commits</h1><div class="sub">Cross-project development activity</div></div></div><div class="empty">Loading commits across managed projects…</div>');};
 async function load(){loading();const started=Date.now();try{[commits,me]=await Promise.all([client().listRecentCommits({perPage:50,scope:branchScope}),client().currentUser().catch(()=>'' )]);}catch(e){p.webview.html=shell(`<div class="empty">${esc(e.message||e)}</div>`);return;}render();liveClient?.log?.(`[perf] Commit dashboard ${Date.now()-started}ms commits=${commits.length}`);}
 function render(){const projects=[...new Set(commits.map(c=>c.repoName))].sort();const data=JSON.stringify(commits).replace(/</g,'\\u003c');p.webview.html=shell(`<div class="top"><div><h1>Commits</h1><div class="sub">${commits.length} recent commits across ${projects.length} project${projects.length===1?'':'s'} · exact times in your local timezone</div></div><button id="refresh">↻ Refresh</button></div><div class="filters"><input id="search" placeholder="Search commit, SHA, author, project, branch, MR…"><select id="scope"><option value="default">Default branches</option><option value="mrs">Open MR branches</option><option value="all">All activity</option></select><select id="project"><option value="">All projects</option>${projects.map(x=>`<option>${esc(x)}</option>`).join('')}</select><select id="author"><option value="">All authors</option><option value="__mine">Mine${me?` (${esc(me)})`:''}</option>${[...new Set(commits.map(c=>c.author).filter(Boolean))].sort().map(x=>`<option>${esc(x)}</option>`).join('')}</select><button id="mine" class="mine">Mine</button><button id="clear">Clear filters</button></div><div class="head"><span>TIME</span><span>PROJECT</span><span>COMMIT</span><span class="author">AUTHOR</span><span>MESSAGE</span></div><div id="rows"></div><script>const vscode=acquireVsCodeApi(),all=${data},me=${JSON.stringify(me||'')};const rows=document.getElementById('rows'),search=document.getElementById('search'),scope=document.getElementById('scope'),project=document.getElementById('project'),author=document.getElementById('author');scope.value=${JSON.stringify(branchScope)};const e=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const dayKey=d=>{const x=new Date(d);return x.getFullYear()+'-'+String(x.getMonth()+1).padStart(2,'0')+'-'+String(x.getDate()).padStart(2,'0')};const relativeClient=d=>{const s=Math.max(0,Math.floor((Date.now()-new Date(d).getTime())/1000));if(s<60)return 'just now';if(s<3600)return Math.floor(s/60)+'m ago';if(s<86400)return Math.floor(s/3600)+'h ago';return Math.floor(s/86400)+'d ago'};const dayLabel=d=>{const x=new Date(d),today=new Date(),y=new Date();y.setDate(today.getDate()-1);if(dayKey(x)===dayKey(today))return 'Today — '+x.toLocaleDateString(undefined,{month:'long',day:'numeric'});if(dayKey(x)===dayKey(y))return 'Yesterday — '+x.toLocaleDateString(undefined,{month:'long',day:'numeric'});return x.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric',year:'numeric'})};function draw(){const q=search.value.toLowerCase().trim(),pv=project.value,av=author.value;let last='';const a=all.filter(c=>(!pv||c.repoName===pv)&&(!av||(av==='__mine'?(!me||c.author.toLowerCase()===me.toLowerCase()):c.author===av))&&(!q||[c.title,c.message,c.shortId,c.id,c.author,c.repoName,...(c.branches||[]),...(c.mrs||[]).flatMap(m=>[m.iid,m.title,m.source,m.target])].join(' ').toLowerCase().includes(q)));rows.innerHTML=a.map(c=>{const day=dayKey(c.created),hdr=day!==last?'<div class="day">'+e(dayLabel(c.created))+'</div>':'';last=day;const exact=new Date(c.created).toLocaleString(undefined,{year:'numeric',month:'long',day:'numeric',hour:'numeric',minute:'2-digit',second:'2-digit'});const dt=new Date(c.created),today=new Date(),y=new Date();y.setDate(today.getDate()-1);const recent=dayKey(dt)===dayKey(today)||dayKey(dt)===dayKey(y);const rel=recent?relativeClient(dt):'';return hdr+'<div class="row" data-id="'+e(c.id)+'" title="'+e(exact)+'"><span class="time">'+e(dt.toLocaleTimeString(undefined,{hour:'numeric',minute:'2-digit'}))+(rel?'<small>'+e(rel)+'</small>':'')+'</span><span class="repo"><span class="repo-badge">'+e(c.repoName)+'</span></span><span class="sha">'+e(c.shortId)+'</span><span class="author">'+e(c.author)+'</span><span class="title"><span>'+e(c.title)+'</span><small class="commit-context">'+(c.branches||[]).map(b=>'<code>⑂ '+e(b)+'</code>').join(' ')+(c.mrs||[]).map(m=>' <button class="mr" data-mr-repo="'+e(c.repo)+'" data-mr-iid="'+e(m.iid)+'">MR !'+e(m.iid)+' · '+e(m.title)+' · '+e(String(m.state||'').toUpperCase())+'</button>').join('')+'</small></span></div><div class="detail" id="d-'+e(c.id)+'"></div>'}).join('')||'<div class="empty">No commits match these filters.</div>';document.querySelectorAll('.row').forEach(x=>x.onclick=()=>{const d=document.getElementById('d-'+x.dataset.id);if(d.classList.contains('open')){d.classList.remove('open');return}d.classList.add('open');if(!d.dataset.loaded){d.innerHTML='Loading commit details and merge request…';vscode.postMessage({type:'details',id:x.dataset.id})}});document.querySelectorAll('[data-mr-iid]').forEach(x=>x.onclick=ev=>{ev.stopPropagation();vscode.postMessage({type:'openMr',repo:x.dataset.mrRepo,iid:Number(x.dataset.mrIid)})})}search.oninput=draw;scope.onchange=()=>vscode.postMessage({type:'scope',scope:scope.value});project.onchange=draw;author.onchange=()=>{document.getElementById('mine').classList.toggle('active',author.value==='__mine');draw()};document.getElementById('mine').onclick=()=>{author.value=author.value==='__mine'?'':'__mine';document.getElementById('mine').classList.toggle('active',author.value==='__mine');draw()};document.getElementById('clear').onclick=()=>{search.value='';project.value='';author.value='';document.getElementById('mine').classList.remove('active');draw()};document.getElementById('refresh').onclick=()=>vscode.postMessage({type:'refresh'});window.addEventListener('message',ev=>{const m=ev.data;if(m.type!=='details')return;const d=document.getElementById('d-'+m.id);if(!d)return;d.dataset.loaded='1';if(m.error){d.innerHTML='<span class="muted">'+e(m.error)+'</span>';return}const c=m.commit,s=c.stats||{},files=c.files||[],mrs=c.mrs||[];d.innerHTML=(mrs.length?'<div class="commit-mrs"><b>Merge request</b> '+mrs.map(x=>'<button class="mr detail-mr" data-mr-repo="'+e(c.repo)+'" data-mr-iid="'+e(x.iid)+'">!'+e(x.iid)+' · '+e(x.title)+' · '+e(String(x.state||'').toUpperCase())+'</button>').join(' ')+'</div>':'<div class="commit-mrs muted">No associated merge request.</div>')+'<div class="stats"><span>'+files.length+' files changed</span><span class="plus">+'+(s.additions??files.reduce((n,f)=>n+(f.added||0),0))+'</span><span class="minus">−'+(s.deletions??files.reduce((n,f)=>n+(f.removed||0),0))+'</span></div><div class="files">'+files.map(f=>'<span class="file">'+e(f.newPath||f.oldPath)+'</span><span><b class="plus">+'+f.added+'</b> <b class="minus">−'+f.removed+'</b></span>').join('')+'</div>';d.querySelectorAll('[data-mr-iid]').forEach(x=>x.onclick=ev=>{ev.stopPropagation();vscode.postMessage({type:'openMr',repo:x.dataset.mrRepo,iid:Number(x.dataset.mrIid)})})});draw();</script>`);}
 p.webview.onDidReceiveMessage(async m=>{if(m.type==='refresh')return load();if(m.type==='scope'){branchScope=String(m.scope||'default');return load();}if(m.type==='openMr'){try{const mr=await client().getMergeRequest(m.repo,Number(m.iid));if(mr)await openMr(client,mr);}catch(e){vscode.window.showErrorMessage(`Could not open merge request !${m.iid}: ${String(e.message||e)}`);}return;}if(m.type==='details'){const c=commits.find(x=>x.id===m.id);if(!c)return;try{const [detail,mrs]=await Promise.all([client().getCommitDetails(c),client().getCommitMergeRequests(c)]);c.mrs=mrs||[];c.mrLookupPending=false;p.webview.postMessage({type:'details',id:m.id,commit:{...detail,mrs:c.mrs}});}catch(e){p.webview.postMessage({type:'details',id:m.id,error:String(e.message||e)});}}});
 await load();
}
...
async function openMr(client,mr,options={}){
 let full=mr,discussions=[],commits=[],loaded=false,impactLoaded=false,impactData=new Map();let activeTab=options.tab||'conversation';const focusDiscussionId=options.discussionId?String(options.discussionId):'';let readiness={approvals:{approved:0,required:0},pipeline:null,jobs:[]};
 ...
 async function loadMrData(force=false){
  ...
  const fullPromise=client().getMergeRequest(mr.repo,mr.iid).catch(()=>full);
  const discussionsPromise=client().listDiscussions(full).catch(()=>full.discussions||[]);
  const readinessPromise=client().getMergeReadiness(full).catch(()=>readiness);
  const commitsPromise=client().listMergeRequestCommits(full).catch(()=>commits);
  const [nextFull,d,r,c]=await Promise.all([fullPromise,discussionsPromise,readinessPromise,commitsPromise]);
  full=nextFull||full;discussions=d||[];readiness=r||readiness;commits=c||[];loaded=true;
 ...
 async function render(tab=activeTab,force=false){
  ...
  const conversation=general.length?general.map(d=>renderConversationThread(d,focusDiscussionId)).join(''):'<div class="empty-state"><b>No conversation yet</b><span>General merge request comments will appear here.</span></div>';
  ...
  <div class="mr-layout"><main>
   ...
   <section class="tab-panel ${activeTab==='feedback'?'show':''}">
    ${renderFeedbackDashboard(feedbackOpen,feedbackResolved,focusDiscussionId)}
 ...
 p.webview.onDidReceiveMessage(async m=>{
  ...
  if(m.action==='review'){await startReview(client,full);return;}if(m.action){await action(client,full,m.action);if(m.action==='approve'){loaded=false;await render(activeTab,true);}else await render(activeTab);return;}if(Number.isInteger(m.file)){await openDemoDiff(full,full.files[m.file]);return;}
...
await openSourceFeedback(d);reviewTree.setFeedback(sourceFeedback);}catch(err){vscode.window.showErrorMessage(`Could not open feedback in project source: ${String(err.stderr||err.message||err)}`);}}return;}
  if(m.feedbackReply){const d=discussions.find(x=>String(x.id)===String(m.feedbackReply));if(!d)return;const body=await vscode.window.showInputBox({title:`Reply to ${d.notes?.[0]?.author||'reviewer'}`,prompt:d.path?`${d.path}${d.line?`:${d.line}`:''}`:'General discussion',placeHolder:'Reply…',ignoreFocusOut:true});if(!body?.trim())return;try{await client().replyDiscussion(full,d.id,body.trim());loaded=false;impactLoaded=false;await render(activeTab,true);mrWebview?.refresh();vscode.window.setStatusBarMessage('GitLab reply posted.',2500);}catch(e){vscode.window.showErrorMessage(`Could not reply to GitLab feedback: ${String(e.stderr||e.message||e)}`);}return;}
...
function renderFeedbackImpact(discussions,files,commits,impactData=new Map()){
 const positioned=discussions.filter(d=>d.path&&d.resolvable),open=positioned.filter(d=>!d.resolved),resolved=positioned.filter(d=>d.resolved);
 const fileMap=new Map((files||[]).map((f,i)=>[f[0]||f[3]?.new_path||f[3]?.old_path,{f,i}]));
 const renderGroups=(items,sectionClass='')=>{
  const byFile=new Map();
  for(const d of items){const key=d.path||d.newPath||d.oldPath;if(!byFile.has(key))byFile.set(key,[]);byFile.get(key).push(d);}
  return [...byFile.entries()].map(([file,threads])=>{const meta=fileMap.get(file),f=meta?.f,stats=f?`+${f[1]} −${f[2]}`:'changed';
   return `<section class="impact-file ${sectionClass}"><div class="impact-file-head"><div><b>${esc(file)}</b><span>${esc(stats)} · ${threads.length} feedback location${threads.length===1?'':'s'}</span></div></div>
   ${threads.slice().sort((a,b)=>Number(a.line||a.newLine||a.oldLine||0)-Number(b.line||b.newLine||b.oldLine||0)).map(d=>{const first=d.notes?.[0]||{},replies=(d.notes||[]).slice(1),line=d.line||d.newLine||d.oldLine||1,data=impactData.get(String(d.id)),diff=data?feedbackLineDiff(data.before,data.current,line):null;return `<article class="impact-thread ${d.resolved?'resolved':''}"><div class="impact-marker">${d.resolved?'✓':'●'}</div><div class="impact-body"><div class="impact-meta"><b>${esc(first.author||'Reviewer')}</b><span>Reviewed line ${esc(line)}</span><span>${d.resolved?'Resolved':'Needs reviewer verification'}</span></div><div class="impact-comment">${formatText(first.body||'Review feedback')}</div>${replies.length?`<div class="impact-replies">${replies.map(n=>`<div><b>${esc(n.author||'Author')}</b> ${formatText(n.body||'')}</div>`).join('')}</div>`:''}${diff?`<div class="impact-diff"><div class="impact-diff-title"><span><b>At feedback</b><small>reviewed line ${line}</small></span><span><b>Current MR</b><small>matched near line ${diff.currentLine}</small></span><em class="impact-confidence ${diff.confidence}">${diff.confidence} confidence</em></div><div class="impact-diff-sides"><div>${diff.left}</div><div>${diff.right}</div></div></div>`:`<div class="impact-no-diff">The reviewed file version could not be reconstructed for this feedback.</div>`}<div class="impact-actions"><button class="secondary" data-feedback-reply="${esc(d.id)}">↩ Reply</button><button class="secondary" data-feedback-diff="${esc(d.id)}">Open feedback diff</button><button class="secondary" data-feedback-open="${esc(d.id)}">Open changed location</button>${!d.resolved?`<button class="primary" data-feedback-resolve="${esc(d.id)}">Resolve after verification</button>`:''}</div></div></article>`}).join('')}</section>`;
  }).join('');
 };
 const openGroups=renderGroups(open);
 const resolvedGroups=renderGroups(resolved,'resolved-impact-file');
 const affected=new Set(positioned.map(d=>d.path||d.newPath||d.oldPath)).size;
 return `<div class="feedback-hero"><div><h2>Feedback Impact</h2><div class="feedback-strip"><b>${open.length}</b> awaiting reviewer verification <i>·</i> <b>${affected}</b> affected file${affected===1?'':'s'} <i>·</i> <b>${resolved.length}</b> resolved</div><p class="impact-help">Outstanding feedback is shown first. Resolved feedback is collapsed below so the work that still needs verification stays at the top.</p></div></div>
 ${openGroups||'<div class="empty-state"><b>No feedback awaiting verification</b><span>All positioned feedback is currently resolved.</span></div>'}
 ${resolved.length?`<details class="resolved-impact"><summary>Resolved feedback <span>${resolved.length}</span></summary><div class="resolved-impact-body">${resolvedGroups}</div></details>`:''}`;
}
function renderFeedbackDashboard(open,resolved,focusId=''){
 ...
 <div class="reviewer-list">${active.length?active.map(([name,v])=>renderReviewerFeedback(name,v,focusId)).join(''):'<div class="empty-state"><b>No outstanding feedback</b><span>All resolvable reviewer feedback has been addressed.</span></div>'}</div>
...
function renderReviewerFeedback(name,data,focusId=''){
 const byFile=new Map();
 ...
 ${[...byFile.entries()].map(([file,items])=>`<div class="feedback-file"><div class="feedback-file-head"><b>${esc(file)}</b><span>${items.length}</span></div>${items.map(d=>renderFeedbackLine(d,focusId)).join('')}</div>`).join('')}
 </section>`;
}
function renderFeedbackLine(d,focusId=''){
 const notes=d.notes||[],first=notes[0]||{},loc=d.line?`Line ${d.line}`:'General',focused=String(d.id)===String(focusId);
 const thread=notes.map((n,i)=>`<div class="feedback-note ${i?'reply-note':''}"><div class="feedback-note-head"><b>${esc(n.author||'Reviewer')}</b><small>${friendlyTime(n.created)}</small></div><div class="feedback-note-body">${formatText(n.body||'')}</div></div>`).join('');
 return `<div class="feedback-thread-card ${focused?'watcher-focus':''}" data-feedback-id="${esc(d.id)}"><button class="feedback-thread-open" data-feedback-open="${esc(d.id)}"><span class="feedback-loc">${esc(loc)}</span><span>Open in source ›</span></button>${thread}<div class="impact-actions"><button class="secondary" data-feedback-reply="${esc(d.id)}">↩ Reply</button></div></div>`;
}
function initials(name){return esc(String(name||'?').split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase()||'?');}
function renderConversationThread(d,focusId=''){const notes=d.notes||[];if(!notes.length)return '';const first=notes[0];const focused=String(d.id)===String(focusId);const replies=notes.slice(1).map(n=>`<div class="reply"><div class="avatar small">${initials(n.author)}</div><div class="reply-content"><div class="reply-head"><b>${esc(n.author||'Reviewer')}</b><small>${friendlyTime(n.created)}</small></div><div class="prose">${formatText(n.body)}</div></div></div>`).join('');return `<article class="thread ${focused?'watcher-focus':''}" data-discussion-id="${esc(d.id)}"><div class="thread-main"><div class="avatar">${initials(first.author)}</div><div class="thread-content"><div class="thread-head"><b>${esc(first.author||'Reviewer')}</b><small>${friendlyTime(first.created)}</small></div><div class="prose">${formatText(first.body)}</div>${replies}<button class="reply-button" data-reply-discussion="${esc(d.id)}">Reply</button></div></div></article>`;}
...
async function submitReview(client){
 ...
 try{
  const reviewedMr=review.mr;
  const result=await client().submitReview(reviewedMr,{summary,outcome:outcome.value});
  const reviewedHead=await client().markReviewed?.(reviewedMr);
  reviewedMr.reviewedHead=reviewedHead||reviewVersion(reviewedMr);reviewedMr.changesSinceReview=0;reviewedMr.changesSinceMyComment=0;
  review.pending=[];review.discussions=await loadDiscussions(client,reviewedMr);review.pending=await loadDraftNotes(client,reviewedMr);
  // We are inside the dedicated Review Explorer now. Do not refresh the MR-list
  // webview here: it may have been disposed when reviewActive hid the normal MR view.
  // The list will refresh when the user returns to it.
  tree.refresh();
  try{reviewTree.refresh();}catch(err){liveClient?.log?.(`[review-flow] post-submit explorer refresh skipped: ${String(err?.message||err)}`);}
  try{await showReviewFile();}catch(err){liveClient?.log?.(`[review-flow] post-submit file refresh skipped: ${String(err?.message||err)}`);}
  vscode.window.showInformationMessage(result.message);
 }catch(err){vscode.window.showErrorMessage(`Could not submit review: ${String(err.stderr||err.message||err)}`);}
}
...
function reviewFileInfo(file){
 const info=file?.[3]||{},display=file?.[0]||info.new_path||info.old_path||'file';
 const oldPath=info.old_path||info.oldPath||display,newPath=info.new_path||info.newPath||display;
 return {display,oldPath,newPath,renamed:oldPath!==newPath};
}
async function applyReviewDiffWhitespace(){
 const desired=!!review.ignoreWhitespace;
 const cfg=vscode.workspace.getConfiguration('diffEditor'),current=cfg.get('ignoreTrimWhitespace',true);
 if(current!==desired)await cfg.update('ignoreTrimWhitespace',desired,vscode.ConfigurationTarget.Workspace);
}
