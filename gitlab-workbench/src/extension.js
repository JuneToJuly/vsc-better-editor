...
function renderFeedbackLine(d,focusId=''){
 ...
 return `<div class="feedback-thread-card ${focused?'watcher-focus':''}" data-feedback-id="${esc(d.id)}"><button class="feedback-thread-open" data-feedback-open="${esc(d.id)}"><span class="feedback-loc">${esc(loc)}</span><span>Open in source ›</span></button>${thread}</div>`;
}
