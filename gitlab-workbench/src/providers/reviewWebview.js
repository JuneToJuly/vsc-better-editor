...
class ReviewWebviewProvider{
 ...
 resolveWebviewView(view){
  ...
  view.webview.onDidReceiveMessage(m=>{
   ...
   if(m.type==='commitRange')vscode.commands.executeCommand('gitlabWorkbench.selectReviewCommit',m.value,!!m.extend);
   if(m.type==='rangeShortcut')vscode.commands.executeCommand('gitlabWorkbench.selectReviewRangeShortcut',m.value);
  });
  this.refresh();
 }
...
function render(review,isReviewed,filter='unresolved',showGeneral=true){
 ...
 const all=review.discussions||[],pending=review.pending||[],files=review.compareFiles||mr.files||[];
 ...
 const rawCommits=review.commits||[],refs=mr.diffRefs||{},target=String(refs.base_sha||refs.baseSha||''),head=String(refs.head_sha||refs.headSha||mr.sha||mr.headSha||'');
 const chronological=rawCommits.length&&String(rawCommits[0]?.id)===head?rawCommits.slice().reverse():rawCommits.slice();
 const commits=chronological;
 const reviewedHead=mr.reviewedHead||'';
 const rangeBase=review.compareBase||target,rangeHead=review.compareHead||head;
 const inRange=id=>{
  const a=chronological.findIndex(c=>String(c.id)===String(rangeBase)),b=chronological.findIndex(c=>String(c.id)===String(rangeHead)),i=chronological.findIndex(c=>String(c.id)===String(id));
  if(i<0)return false;if(a<0)return b>=0&&i<=b;return i>a&&i<=b;
 };
 const stack=commits.slice().reverse().map((c,ri)=>{
  const i=chronological.findIndex(x=>String(x.id)===String(c.id)),prev=i>0?chronological[i-1]:null,selected=inRange(c.id),isHead=String(c.id)===head;
  return `<button class="commit-row ${selected?'selected':''}" data-commit-range="${esc(c.id)}" title="Click: this commit only. Shift+click: extend the selected stack."><span class="commit-dot">●</span><span class="commit-sha">${esc(c.shortId||String(c.id).slice(0,8))}</span><span class="commit-title">${esc(oneLine(c.title||''))}</span>${isHead?'<span class="badge current">HEAD</span>':''}</button>`;
 }).join('');
 let body=`<div class="summary"><div class="summary-title"><b>Review !${esc(mr.iid)}</b><span class="repo-name">${esc(mr.repoName||mr.repo||'')}</span></div><div class="stats"><span><b>${done}/${files.length}</b> files reviewed</span><span><b>${unresolved}</b> unresolved</span>${pending.length?`<span><b>${pending.length}</b> pending</span>`:''}</div><div class="commit-tools"><button class="chip" data-range-shortcut="all">All MR changes</button>${reviewedHead?'<button class="chip" data-range-shortcut="reviewed">Since my last review</button>':''}<span class="range-label">${esc(review.compareLabel||'')}</span></div><div class="commit-stack">${stack}<button class="commit-row target-row" data-range-shortcut="all"><span class="commit-dot">○</span><span class="commit-sha">TARGET</span><span class="commit-title">${esc(target.slice(0,8))}</span></button></div></div>
 ...
 files.forEach(f=>{const index=(mr.files||[]).findIndex(x=>x[0]===f[0]);const parts=f[0].split('/'),dir=parts.length>1?parts.slice(0,-1).join('/'):'(root)';if(!groups.has(dir))groups.set(dir,[]);groups.get(dir).push({f,index});});
 if(!files.length)body+=`<div class="empty">No files changed in this comparison.</div>`;
...
.commit-tools{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:8px}.range-label{font-size:11px;color:var(--vscode-descriptionForeground);margin-left:auto}.commit-stack{margin-top:8px;border:1px solid var(--vscode-panel-border);border-radius:4px;overflow:hidden}.commit-row{width:100%;display:grid;grid-template-columns:18px 70px minmax(0,1fr) auto;align-items:center;gap:5px;text-align:left;border:0;border-bottom:1px solid var(--vscode-panel-border);border-radius:0;background:transparent;padding:5px 7px;color:var(--vscode-foreground);font-size:11px}.commit-row:last-child{border-bottom:0}.commit-row:hover{background:var(--vscode-list-hoverBackground)}.commit-row.selected{background:var(--vscode-list-activeSelectionBackground);color:var(--vscode-list-activeSelectionForeground)}.commit-dot{text-align:center}.commit-sha{font-family:var(--vscode-editor-font-family);font-size:10px}.commit-title{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.target-row{opacity:.8}
</style></head><body>${body}<script>
const vscode=acquireVsCodeApi();const dec=s=>JSON.parse(decodeURIComponent(s));document.querySelectorAll('[data-commit-range]').forEach(el=>el.addEventListener('click',e=>{e.stopPropagation();vscode.postMessage({type:'commitRange',value:el.dataset.commitRange,extend:e.shiftKey});}));
document.querySelectorAll('[data-range-shortcut]').forEach(el=>el.addEventListener('click',e=>{e.stopPropagation();vscode.postMessage({type:'rangeShortcut',value:el.dataset.rangeShortcut});}));
