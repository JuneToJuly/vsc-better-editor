...
function renderFlowReplayHtml(result) {
  ...
  *{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);font-family:var(--vscode-font-family)}
  ...
  .content{min-height:0;flex:1;display:grid;grid-template-rows:minmax(0,1fr) 4px minmax(110px,var(--state-height));overflow:hidden}.content.state-collapsed{grid-template-rows:minmax(0,1fr) 4px 34px}.code-pane{min-height:0;overflow:auto;padding:10px 14px 14px;scrollbar-gutter:stable}.current-head{display:flex;gap:16px;margin-bottom:8px;position:sticky;top:-10px;z-index:3;background:var(--vscode-editor-background);padding:9px 0 7px;border-bottom:1px solid color-mix(in srgb,var(--vscode-panel-border) 55%,transparent)}.identity{min-width:0}.eyebrow{text-transform:uppercase;letter-spacing:.09em;font-size:9px;color:var(--vscode-descriptionForeground);margin-bottom:4px}.identity h1{font:600 18px/1.2 var(--vscode-editor-font-family);margin:0}.identity p{font:11px/1.35 var(--vscode-editor-font-family);color:var(--vscode-descriptionForeground);margin:4px 0 0}.meta{margin-left:auto;display:flex;gap:6px}.pill{border:1px solid color-mix(in srgb,var(--vscode-panel-border) 75%,transparent);border-radius:999px;padding:3px 7px;font-size:10px;color:var(--vscode-descriptionForeground)}
  .code-card,.state-card{border:1px solid color-mix(in srgb,var(--vscode-panel-border) 82%,transparent);border-radius:5px;overflow:hidden;background:color-mix(in srgb,var(--vscode-editor-background) 97%,var(--vscode-sideBar-background))}.code-title,.state-title{display:flex;align-items:center;padding:8px 11px;border-bottom:1px solid color-mix(in srgb,var(--vscode-panel-border) 75%,transparent);font-size:11px;background:var(--vscode-editor-background)}.code-title{position:sticky;top:58px;z-index:2}.code-title span{margin-left:auto;color:var(--vscode-descriptionForeground);font-size:10px}.code{font:13px/1.55 var(--vscode-editor-font-family);padding:5px 0;overflow:auto;max-width:100%;scrollbar-gutter:stable}.line{display:grid;grid-template-columns:46px max-content;min-width:100%;padding:0 10px;border-left:3px solid transparent}.line:not(.covered):not(.active){opacity:.62}.line.covered{background:color-mix(in srgb,var(--vscode-testing-iconPassed) 4%,transparent);border-left-color:color-mix(in srgb,var(--vscode-testing-iconPassed) 52%,transparent)}.line.covered .num{color:var(--vscode-testing-iconPassed)}.line.active{background:var(--vscode-editor-lineHighlightBackground);border-left-color:var(--vscode-focusBorder);opacity:1}.line.active.covered{box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--vscode-testing-iconPassed) 18%,transparent)}.num{color:var(--vscode-editorLineNumber-foreground);text-align:right;padding-right:12px;position:sticky;left:0;background:inherit}.line.active .num{color:var(--vscode-editorLineNumber-activeForeground);font-weight:700}.text{white-space:pre}
...
</style></head><body><div class="shell" id="shell"><aside class="sidebar"><div class="side-head"><strong>Call tree</strong><small>Browse invocations, call sites, returns, and state changes.</small><div class="tabs"><button id="treeTab" class="active">Calls</button><button id="timelineTab">Timeline</button><button id="changesTab">Changes</button></div></div><div class="tree" id="tree"></div></aside><div class="v-splitter" id="sideSplitter" title="Drag to resize call tree"></div><main class="viewer"><div class="toolbar"><button id="prev">← Previous</button><button class="primary" id="next">Next →</button><button id="open">Open source</button><span class="position" id="position"></span></div><div class="content" id="content"><div class="code-pane" id="codePane"><div class="current-head"><div class="identity"><div class="eyebrow" id="kind"></div><h1 id="method"></h1><p id="file"></p></div><div class="meta"><span class="pill" id="depth"></span><span class="pill" id="thread"></span></div></div><div class="code-card" id="codeCard"><div class="code-title"><strong id="codeFile"></strong><span id="codeLine"></span></div><div class="code" id="code"></div></div><div class="empty" id="empty" style="display:none">Source preview is unavailable for this boundary.</div></div><div class="h-splitter" id="stateSplitter" title="Drag to resize invocation state"></div><div class="state-pane" id="statePane"><div class="state-card"><div class="state-title" id="stateTitle" title="Collapse or expand invocation state">Invocation state</div><div class="compare-grid"><section><h3>Before</h3><div id="before"></div></section><section><h3>After</h3><div id="after"></div></section><section><h3>Changes / Outcome</h3><div id="changes"></div></section></div></div><div class="hint"><kbd>j</kbd>/<kbd>k</kbd> move through boundaries. <kbd>Enter</kbd> opens source.</div></div></div></main></div><script>
  ...
  const eventIndex=new Map(events.map((e,i)=>[e,i]));const expanded=new Set(roots.map(r=>r.id));
  ...
  function resultText(n){if(!n.exit)return 'running';if(n.exit.thrown)return 'throws '+valueText(n.exit.thrown);return '→ '+valueText(n.exit.returnValue)}
  ...
  function boundaryButton(e,icon,label){if(!e)return null;const b=document.createElement('button');b.className='boundary-row '+(e.event||'');b.dataset.event=eventIndex.get(e);b.innerHTML='<span class="icon">'+icon+'</span><span class="label"><b>'+label+'</b><small>'+((e.sourceFile||simpleName(e)+'.java')+(e.line?':'+e.line:''))+'</small></span>';b.onclick=()=>selectEvent(e);return b}
  function nodeElement(n,depth=0,changesOnly=false){const wrap=document.createElement('div');const changed=diffNode(n);if(changesOnly&&!changed&&!n.exit?.thrown)return wrap;const row=document.createElement('button');row.className='node-row'+(changed?' changed-node':'');row.style.paddingLeft=(10+depth*14)+'px';const hasBoundaries=!!(n.callsite||n.entry||n.exit||n.resume);const expandable=hasBoundaries||n.children.length>0;const isOpen=expanded.has(n.id);const entry=n.entry||n.callsite||n.exit;row.innerHTML='<span class="twisty">'+(expandable?(isOpen?'▾':'▸'):'')+'</span><span class="icon">◇</span><span class="label"><b>'+simpleName(entry)+'.'+(entry?.methodName||'unknown')+'() <span class="summary">'+resultText(n)+'</span></b><small>'+((n.callsite?.sourceFile||entry?.sourceFile||'')+(n.callsite?.line?':'+n.callsite.line:''))+(changed?' · '+changed+' field'+(changed===1?'':'s')+' changed':'')+'</small></span>';row.onclick=()=>{if(expandable){isOpen?expanded.delete(n.id):expanded.add(n.id)}selectEvent(n.callsite||n.entry||n.exit);renderTree(changesOnly)};wrap.appendChild(row);if(isOpen&&expandable){const kids=document.createElement('div');kids.className='children';const parts=[[n.callsite,'◇','Call site'],[n.entry,'▶','Method entry']];for(const [e,i,l] of parts){const b=boundaryButton(e,i,l);if(b)kids.appendChild(b)}for(const child of n.children)kids.appendChild(nodeElement(child,depth+1,changesOnly));for(const [e,i,l] of [[n.exit,n.exit?.thrown?'⚠':'↩',n.exit?.thrown?'Exception':'Method return'],[n.resume,'←','Caller resumes']]){const b=boundaryButton(e,i,l);if(b)kids.appendChild(b)}wrap.appendChild(kids)}return wrap}
  ...
  function renderDetails(){const e=events[selectedEvent];document.getElementById('kind').textContent=e.event==='callsite'?'Call site':e.event==='enter'?'Method entry':e.event==='exit'?'Method return':e.event==='resume'?'Caller resumes':'Executed line';document.getElementById('method').textContent=e.event==='callsite'?'Calls '+simpleName({className:e.calleeClassName})+'.'+e.calleeMethodName+'()':e.event==='resume'?'Returns to '+simpleName(e)+'.'+e.methodName+'()':simpleName(e)+'.'+e.methodName+'()';document.getElementById('file').textContent=(e.sourceFile||e.className)+(e.line?':'+e.line:'');document.getElementById('depth').textContent='Depth '+Number(e.depth||0);document.getElementById('thread').textContent='Thread '+String(e.thread||e.threadId||'main');document.getElementById('position').textContent=(selectedEvent+1)+' of '+events.length;document.getElementById('prev').disabled=selectedEvent===0;document.getElementById('next').disabled=selectedEvent===events.length-1;renderCode(e);renderState(e)}
