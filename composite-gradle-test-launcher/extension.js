...
function renderFlowReplayHtml(result) {
  ...
  for (const event of rawEntries) {
    ...
    if (event.event === 'exit' && hasCaller) {
      entries.push({
        ...
        callerReceiverAfter: event.callerReceiverAfter,
        callerArgumentsAfter: event.callerArgumentsAfter,
        __synthetic: true
      });
    }
  }
  ...
  *{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);font-family:var(--vscode-font-family)}
  .shell{--sidebar-width:390px;--state-height:235px;height:100vh;display:grid;grid-template-columns:var(--sidebar-width) 4px minmax(0,1fr);overflow:hidden}
  .sidebar{background:var(--vscode-sideBar-background);display:flex;flex-direction:column;min-width:0;overflow:hidden;border-right:1px solid var(--vscode-panel-border)}
  .v-splitter,.h-splitter{background:transparent;position:relative;z-index:5}.v-splitter:after,.h-splitter:after{content:'';position:absolute;background:var(--vscode-panel-border)}.v-splitter:after{width:1px;top:0;bottom:0;left:1px}.h-splitter:after{height:1px;left:0;right:0;top:1px}.v-splitter{cursor:col-resize}.h-splitter{cursor:row-resize}.v-splitter:hover,.h-splitter:hover,.v-splitter.dragging,.h-splitter.dragging{background:color-mix(in srgb,var(--vscode-focusBorder) 25%,transparent)}
  .side-head{padding:13px 14px 10px;border-bottom:1px solid var(--vscode-panel-border);flex:0 0 auto;background:color-mix(in srgb,var(--vscode-sideBar-background) 92%,var(--vscode-editor-background))}.side-head strong{font-size:13px}.side-head small{display:block;color:var(--vscode-descriptionForeground);margin-top:3px;font-size:11px}.tabs{display:flex;gap:2px;margin-top:10px;padding:2px;background:color-mix(in srgb,var(--vscode-editor-background) 55%,transparent);border-radius:5px;width:max-content}.tabs button{border:0;background:transparent;color:var(--vscode-descriptionForeground);padding:5px 9px;cursor:pointer;border-radius:3px;font-size:11px}.tabs button.active{background:var(--vscode-list-activeSelectionBackground);color:var(--vscode-list-activeSelectionForeground)}
  .tree{overflow:auto;min-height:0;flex:1;padding:6px 0 14px;scrollbar-gutter:stable}.node-row,.boundary-row{width:100%;border:0;background:transparent;color:inherit;text-align:left;cursor:pointer;display:grid;align-items:center;border-left:2px solid transparent}.node-row{grid-template-columns:14px 16px 1fr;padding:6px 9px}.boundary-row{grid-template-columns:19px 1fr;padding:4px 9px 4px 22px;color:var(--vscode-descriptionForeground);opacity:.68}.boundary-row.callsite,.boundary-row.resume{opacity:.88}.boundary-row.exit{opacity:.82}.node-row:hover,.boundary-row:hover{background:var(--vscode-list-hoverBackground);opacity:1}.node-row.active,.boundary-row.active{background:var(--vscode-list-activeSelectionBackground);color:var(--vscode-list-activeSelectionForeground);border-left-color:var(--vscode-focusBorder);opacity:1}.node-row.changed-node .summary{color:var(--vscode-editorWarning-foreground)}.twisty{font-size:10px;color:var(--vscode-descriptionForeground)}.icon{color:var(--vscode-textLink-foreground);font-size:11px}.boundary-row.callsite .icon{color:var(--vscode-descriptionForeground)}.boundary-row.enter .icon{color:var(--vscode-textLink-foreground)}.boundary-row.exit .icon{color:var(--vscode-testing-iconPassed)}.boundary-row.resume .icon{color:var(--vscode-symbolIcon-variableForeground,var(--vscode-textLink-foreground))}.boundary-row.exception .icon{color:var(--vscode-errorForeground)}.label{min-width:0;font-family:var(--vscode-editor-font-family);font-size:12px}.label b{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-weight:600}.boundary-row .label b{font-weight:500}.label small{display:block;opacity:.64;margin-top:1px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:10px}.summary{float:right;color:var(--vscode-descriptionForeground);font-size:10px;font-weight:400;margin-left:8px}.children{border-left:1px solid color-mix(in srgb,var(--vscode-panel-border) 45%,transparent);margin-left:13px}
  .viewer{display:flex;flex-direction:column;min-width:0;min-height:0;overflow:hidden}.toolbar{display:flex;gap:6px;align-items:center;padding:8px 14px;border-bottom:1px solid var(--vscode-panel-border);background:var(--vscode-editorGroupHeader-tabsBackground);flex:0 0 auto}button{font:inherit}.toolbar button{border:1px solid var(--vscode-button-border,transparent);background:var(--vscode-button-secondaryBackground);color:var(--vscode-button-secondaryForeground);padding:5px 10px;cursor:pointer}.toolbar button.primary{background:var(--vscode-button-background);color:var(--vscode-button-foreground)}.toolbar button:disabled{opacity:.4;cursor:default}.position{margin-left:auto;color:var(--vscode-descriptionForeground);font-size:11px}
  .content{min-height:0;flex:1;display:grid;grid-template-rows:minmax(0,1fr) 4px minmax(110px,var(--state-height));overflow:hidden}.content.state-collapsed{grid-template-rows:minmax(0,1fr) 4px 34px}.code-pane{min-height:0;overflow:auto;padding:0 14px 14px;scrollbar-gutter:stable}.current-head{display:flex;gap:16px;margin:0 -14px 8px;position:sticky;top:0;z-index:4;background:var(--vscode-editor-background);padding:10px 14px 8px;border-bottom:1px solid color-mix(in srgb,var(--vscode-panel-border) 70%,transparent);box-shadow:0 1px 0 color-mix(in srgb,var(--vscode-editor-background) 85%,transparent)}.identity{min-width:0}.eyebrow{text-transform:uppercase;letter-spacing:.09em;font-size:9px;color:var(--vscode-descriptionForeground);margin-bottom:3px}.identity h1{font:600 18px/1.2 var(--vscode-editor-font-family);margin:0}.scope-location{margin-top:3px;color:var(--vscode-descriptionForeground);font:10px/1.3 var(--vscode-editor-font-family);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.meta{margin-left:auto;display:flex;gap:6px}.pill{border:1px solid color-mix(in srgb,var(--vscode-panel-border) 75%,transparent);border-radius:999px;padding:3px 7px;font-size:10px;color:var(--vscode-descriptionForeground)}
  .code-card,.state-card{border:1px solid color-mix(in srgb,var(--vscode-panel-border) 82%,transparent);border-radius:5px;overflow:hidden;background:color-mix(in srgb,var(--vscode-editor-background) 97%,var(--vscode-sideBar-background))}.state-title{display:flex;align-items:center;padding:8px 11px;border-bottom:1px solid color-mix(in srgb,var(--vscode-panel-border) 75%,transparent);font-size:11px;background:var(--vscode-editor-background)}.code{font:13px/1.55 var(--vscode-editor-font-family);padding:5px 0;overflow:auto;max-width:100%;scrollbar-gutter:stable}.line{display:grid;grid-template-columns:46px max-content;min-width:100%;padding:0 10px;border-left:3px solid transparent}.line:not(.covered):not(.active){opacity:.62}.line.covered{background:color-mix(in srgb,var(--vscode-testing-iconPassed) 4%,transparent);border-left-color:color-mix(in srgb,var(--vscode-testing-iconPassed) 52%,transparent)}.line.covered .num{color:var(--vscode-testing-iconPassed)}.line.active{background:var(--vscode-editor-lineHighlightBackground);border-left-color:var(--vscode-focusBorder);opacity:1}.line.active.covered{box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--vscode-testing-iconPassed) 18%,transparent)}.num{color:var(--vscode-editorLineNumber-foreground);text-align:right;padding-right:12px;position:sticky;left:0;background:inherit}.line.active .num{color:var(--vscode-editorLineNumber-activeForeground);font-weight:700}.text{white-space:pre}
  .state-pane{min-height:0;overflow:auto;padding:7px 14px 10px;scrollbar-gutter:stable}.state-title{cursor:pointer;position:sticky;top:0;z-index:2;font-weight:600}.state-title:before{content:'▾';margin-right:7px;color:var(--vscode-descriptionForeground)}.content.state-collapsed .state-title:before{content:'▸'}.content.state-collapsed .compare-grid,.content.state-collapsed .hint{display:none}.compare-grid{display:grid;grid-template-columns:minmax(250px,1fr) minmax(250px,1fr) minmax(310px,1.15fr);min-width:780px}.compare-grid section{padding:10px 12px;min-width:0;overflow:auto;border-right:1px solid color-mix(in srgb,var(--vscode-panel-border) 58%,transparent)}.compare-grid section:last-child{border-right:0;background:color-mix(in srgb,var(--vscode-sideBar-background) 20%,transparent)}.compare-grid h3{margin:0 0 8px;font-size:9px;letter-spacing:.1em;text-transform:uppercase;color:var(--vscode-descriptionForeground)}.state-row{display:grid;grid-template-columns:minmax(76px,auto) minmax(0,1fr);gap:9px;margin:4px 0;font-family:var(--vscode-editor-font-family);font-size:11px;line-height:1.35}.state-key{color:var(--vscode-textLink-foreground);white-space:nowrap}.state-value{word-break:break-word;min-width:0}.state-type{display:block;color:var(--vscode-descriptionForeground);font-size:9px;margin-top:1px}.changed{background:color-mix(in srgb,var(--vscode-editorWarning-foreground) 9%,transparent);padding:4px 6px;border-left:2px solid color-mix(in srgb,var(--vscode-editorWarning-foreground) 55%,transparent);border-radius:2px}.unchanged{opacity:.58}.empty{padding:11px;color:var(--vscode-descriptionForeground);border:1px dashed color-mix(in srgb,var(--vscode-panel-border) 70%,transparent);border-radius:4px;font-size:11px}.hint{margin-top:6px;color:var(--vscode-descriptionForeground);font-size:10px}kbd{border:1px solid var(--vscode-panel-border);border-radius:3px;padding:1px 4px;background:var(--vscode-keybindingLabel-background)}
  @media(max-width:1100px){.shell{--sidebar-width:330px}.compare-grid{grid-template-columns:1fr 1fr;min-width:620px}.compare-grid section:last-child{grid-column:1/-1;border-top:1px solid color-mix(in srgb,var(--vscode-panel-border) 58%,transparent)}.identity h1{font-size:16px}.code-pane{padding-left:10px;padding-right:10px}.state-pane{padding-left:10px;padding-right:10px}}
</style></head><body><div class="shell" id="shell"><aside class="sidebar"><div class="side-head"><strong>Call tree</strong><small>Browse invocations, call sites, returns, and state changes.</small><div class="tabs"><button id="treeTab" class="active">Calls</button><button id="timelineTab">Timeline</button><button id="changesTab">Changes</button></div></div><div class="tree" id="tree"></div></aside><div class="v-splitter" id="sideSplitter" title="Drag to resize call tree"></div><main class="viewer"><div class="toolbar"><button id="prev">← Previous</button><button class="primary" id="next">Next →</button><button id="open">Open source</button><span class="position" id="position"></span></div><div class="content" id="content"><div class="code-pane" id="codePane"><div class="current-head"><div class="identity"><div class="eyebrow" id="kind"></div><h1 id="method"></h1><div class="scope-location" id="scopeLocation"></div></div><div class="meta"><span class="pill" id="depth"></span><span class="pill" id="thread"></span></div></div><div class="code-card" id="codeCard"><div class="code" id="code"></div></div><div class="empty" id="empty" style="display:none">Source preview is unavailable for this boundary.</div></div><div class="h-splitter" id="stateSplitter" title="Drag to resize invocation state"></div><div class="state-pane" id="statePane"><div class="state-card"><div class="state-title" id="stateTitle" title="Collapse or expand invocation state">Invocation state</div><div class="compare-grid"><section><h3>Before</h3><div id="before"></div></section><section><h3>After</h3><div id="after"></div></section><section><h3>Changes / Outcome</h3><div id="changes"></div></section></div></div><div class="hint"><kbd>j</kbd>/<kbd>k</kbd> move through boundaries. <kbd>Enter</kbd> opens source.</div></div></div></main></div><script>
  const vscode=acquireVsCodeApi(),events=${data};const savedUi=vscode.getState()||{};let selectedEvent=0,mode='tree';const tree=document.getElementById('tree'),shell=document.getElementById('shell'),content=document.getElementById('content');if(savedUi.sidebarWidth)shell.style.setProperty('--sidebar-width',savedUi.sidebarWidth+'px');if(savedUi.stateHeight)shell.style.setProperty('--state-height',savedUi.stateHeight+'px');if(savedUi.stateCollapsed)content.classList.add('state-collapsed');
  const simpleName=e=>String(e.className||'').split('.').pop();const callKey=e=>String(e.callId??e.call??e.invocationId??('seq-'+e.sequence));
  const calls=new Map(),roots=[],stackByThread=new Map();
  for(const e of events){const key=callKey(e);if(e.event==='callsite'){const n=calls.get(key)||{id:key,children:[]};n.callsite=e;calls.set(key,n);}if(e.event==='enter'){const n=calls.get(key)||{id:key,children:[]};n.entry=e;calls.set(key,n);const thread=String(e.thread??e.threadId??'main'),stack=stackByThread.get(thread)||[];const parent=stack[stack.length-1];if(parent){n.parent=parent;parent.children.push(n)}else roots.push(n);stack.push(n);stackByThread.set(thread,stack);}if(e.event==='exit'){const n=calls.get(key)||{id:key,children:[]};n.exit=e;calls.set(key,n);const thread=String(e.thread??e.threadId??'main'),stack=stackByThread.get(thread)||[];const idx=stack.lastIndexOf(n);if(idx>=0)stack.splice(idx,1);}if(e.event==='resume'){const n=calls.get(key)||{id:key,children:[]};n.resume=e;calls.set(key,n);}}
  const eventIndex=new Map(events.map((e,i)=>[e,i]));const expanded=new Set();
  ...
  function fieldsOf(v){return v&&v.fields&&typeof v.fields==='object'?v.fields:{}}
  function diffNode(n){const before=fieldsOf(n.entry&&n.entry.receiver),after=fieldsOf(n.exit&&n.exit.receiverAfter),keys=new Set([...Object.keys(before),...Object.keys(after)]);let count=0;for(const k of keys)if(valueText(before[k])!==valueText(after[k]))count++;return count}
  function resultText(n){if(!n.exit)return '…';if(n.exit.thrown)return '⚠ '+valueText(n.exit.thrown);const value=valueText(n.exit.returnValue);return '↩ '+value}
  function autoExpand(n,isRoot=false){if(isRoot||diffNode(n)>0||n.exit?.thrown)expanded.add(n.id);for(const child of n.children)autoExpand(child,false)}for(const root of roots)autoExpand(root,true);
  function selectEvent(e){if(!e)return;selectedEvent=eventIndex.get(e)??0;renderDetails();document.querySelectorAll('[data-event]').forEach(x=>x.classList.toggle('active',Number(x.dataset.event)===selectedEvent))}
  function boundaryButton(e,icon,label){if(!e)return null;const b=document.createElement('button');b.className='boundary-row '+(e.event||'')+(e.thrown?' exception':'');b.dataset.event=eventIndex.get(e);const detail=e.event==='exit'?(e.thrown?'threw '+valueText(e.thrown):'returned '+valueText(e.returnValue)):((e.sourceFile||simpleName(e)+'.java')+(e.line?':'+e.line:''));b.innerHTML='<span class="icon">'+icon+'</span><span class="label"><b>'+label+'</b><small>'+detail+'</small></span>';b.onclick=()=>selectEvent(e);return b}
  function nodeElement(n,depth=0,changesOnly=false){const wrap=document.createElement('div');const changed=diffNode(n);if(changesOnly&&!changed&&!n.exit?.thrown)return wrap;const row=document.createElement('button');row.className='node-row'+(changed?' changed-node':'');row.style.paddingLeft=(8+depth*10)+'px';const hasBoundaries=!!(n.callsite||n.entry||n.exit||n.resume);const expandable=hasBoundaries||n.children.length>0;const isOpen=expanded.has(n.id);const entry=n.entry||n.callsite||n.exit;row.innerHTML='<span class="twisty">'+(expandable?(isOpen?'▾':'▸'):'')+'</span><span class="icon">◇</span><span class="label"><b>'+simpleName(entry)+'.'+(entry?.methodName||'unknown')+'() <span class="summary">'+resultText(n)+'</span></b><small>'+((n.callsite?.sourceFile||entry?.sourceFile||'')+(n.callsite?.line?':'+n.callsite.line:''))+(changed?' · '+changed+' change'+(changed===1?'':'s'):'')+'</small></span>';row.onclick=()=>{if(expandable){isOpen?expanded.delete(n.id):expanded.add(n.id)}selectEvent(n.callsite||n.entry||n.exit);renderTree(changesOnly)};wrap.appendChild(row);if(isOpen&&expandable){const kids=document.createElement('div');kids.className='children';for(const [e,i,l] of [[n.callsite,'◇','Call'],[n.entry,'▶','Entry']]){const b=boundaryButton(e,i,l);if(b)kids.appendChild(b)}for(const child of n.children)kids.appendChild(nodeElement(child,depth+1,changesOnly));for(const [e,i,l] of [[n.exit,n.exit?.thrown?'⚠':'↩',n.exit?.thrown?'Exception':'Exit'],[n.resume,'←','Resume']]){const b=boundaryButton(e,i,l);if(b)kids.appendChild(b)}wrap.appendChild(kids)}return wrap}
  function renderTree(changesOnly=false){tree.innerHTML='';for(const r of roots)tree.appendChild(nodeElement(r,0,changesOnly));document.querySelectorAll('[data-event]').forEach(x=>x.classList.toggle('active',Number(x.dataset.event)===selectedEvent))}
  function renderTimeline(){tree.innerHTML='';events.forEach((e,i)=>{const b=document.createElement('button');b.className='boundary-row '+(e.event||'');b.dataset.event=i;b.innerHTML='<span class="icon">'+(e.event==='callsite'?'◇':e.event==='enter'?'▶':e.event==='exit'?'↩':e.event==='resume'?'←':'•')+'</span><span class="label"><b>'+simpleName(e)+'.'+e.methodName+'()</b><small>'+((e.sourceFile||'')+(e.line?':'+e.line:''))+'</small></span>';b.onclick=()=>selectEvent(e);tree.appendChild(b)});document.querySelectorAll('[data-event]').forEach(x=>x.classList.toggle('active',Number(x.dataset.event)===selectedEvent))}
  function renderCode(e){const code=document.getElementById('code'),card=document.getElementById('codeCard'),empty=document.getElementById('empty'),pane=document.getElementById('codePane');code.innerHTML='';if(!e.sourcePreview||!e.sourcePreview.length){card.style.display='none';empty.style.display='block';return}card.style.display='block';empty.style.display='none';document.getElementById('scopeLocation').textContent=(e.sourceFile||simpleName(e)+'.java')+(e.line?':'+e.line:'');let activeEl=null;for(const row of e.sourcePreview){const div=document.createElement('div');div.className='line'+(row.covered?' covered':'')+(row.active?' active':'');div.innerHTML='<span class="num">'+row.line+'</span><span class="text"></span>';div.querySelector('.text').textContent=row.text||' ';div.onclick=()=>vscode.postMessage({command:'openLine',className:e.className,methodName:e.methodName,line:row.line,sourcePath:e.sourcePath});code.appendChild(div);if(row.active)activeEl=div}requestAnimationFrame(()=>{if(activeEl){const target=activeEl.offsetTop-pane.clientHeight/2+activeEl.clientHeight/2;pane.scrollTo({top:Math.max(0,target),behavior:'smooth'})}})}
  function addState(root,key,v,cls=''){const row=document.createElement('div');row.className='state-row '+cls;row.innerHTML='<span class="state-key"></span><span class="state-value"></span>';row.children[0].textContent=key;row.children[1].textContent=valueText(v);if(v&&v.type){const t=document.createElement('span');t.className='state-type';t.textContent=v.type;row.children[1].appendChild(t)}root.appendChild(row)}
  function renderSnapshot(root,value,label='this'){root.innerHTML='';if(value===undefined||value===null){root.innerHTML='<div class="empty">Not available</div>';return}addState(root,label,value);for(const [k,v] of Object.entries(fieldsOf(value)))addState(root,k,v)}
  function currentNode(e){return calls.get(callKey(e))}
  function renderState(e){const n=currentNode(e)||{};const beforeRoot=document.getElementById('before'),afterRoot=document.getElementById('after'),changesRoot=document.getElementById('changes');if(e.event==='callsite'){document.getElementById('stateTitle').textContent='Caller state across invocation';const callerBefore=e.callerReceiver;const callerAfter=e.callerReceiverAfter;renderSnapshot(beforeRoot,callerBefore,'caller before call');renderSnapshot(afterRoot,callerAfter,'caller after return');changesRoot.innerHTML='';const bf=fieldsOf(callerBefore),af=fieldsOf(callerAfter),keys=new Set([...Object.keys(bf),...Object.keys(af)]);let changed=0;for(const k of keys){const a=valueText(bf[k]),b=valueText(af[k]);if(a!==b){addState(changesRoot,k,a+'  →  '+b,'changed');changed++}}if(!changed&&callerBefore&&callerAfter){const note=document.createElement('div');note.className='empty';note.textContent='No caller receiver fields changed during this invocation';changesRoot.appendChild(note)}const target=e.targetReceiver??e.receiver;if(target){const h=document.createElement('h3');h.textContent='Callee target';changesRoot.appendChild(h);addState(changesRoot,'target this',target)}if(e.callerArguments?.length){const h=document.createElement('h3');h.textContent='Caller arguments';changesRoot.appendChild(h);e.callerArguments.forEach((v,i)=>addState(changesRoot,'caller arg'+i,v))}if(e.arguments?.length){const h=document.createElement('h3');h.textContent='Callee arguments';changesRoot.appendChild(h);e.arguments.forEach((v,i)=>addState(changesRoot,'arg'+i,v))}if(e.thrown||e.returnValue){const h=document.createElement('h3');h.textContent='Callee outcome';changesRoot.appendChild(h);addState(changesRoot,e.thrown?'thrown':'return',e.thrown||e.returnValue)}if(!callerBefore){const note=document.createElement('div');note.className='empty';note.textContent='Caller receiver is unavailable because the caller method was not instrumented.';beforeRoot.appendChild(note)}if(!callerAfter){const note=document.createElement('div');note.className='empty';note.textContent='Caller state after return is unavailable for this trace.';afterRoot.appendChild(note)}return}if(e.event==='resume'){document.getElementById('stateTitle').textContent='Caller state after return';renderSnapshot(beforeRoot,e.callerReceiver??n.entry?.callerReceiver,'caller before call');renderSnapshot(afterRoot,e.callerReceiverAfter,'caller after return');changesRoot.innerHTML='';const bf=fieldsOf(e.callerReceiver??n.entry?.callerReceiver),af=fieldsOf(e.callerReceiverAfter),keys=new Set([...Object.keys(bf),...Object.keys(af)]);let changed=0;for(const k of keys){const a=valueText(bf[k]),b=valueText(af[k]);if(a!==b){addState(changesRoot,k,a+'  →  '+b,'changed');changed++}}if(!changed)changesRoot.innerHTML='<div class="empty">No caller receiver fields changed during this call</div>';const h=document.createElement('h3');h.textContent='Callee outcome';changesRoot.appendChild(h);addState(changesRoot,e.thrown?'thrown':'return',e.thrown||e.returnValue);return}if(e.event==='exit'){document.getElementById('stateTitle').textContent='Method return state';const before=e.__entrySnapshot?.receiver;const after=e.receiverAfter;renderSnapshot(beforeRoot,before,'callee at entry');renderSnapshot(afterRoot,after,'callee at return');changesRoot.innerHTML='';const bf=fieldsOf(before),af=fieldsOf(after),keys=new Set([...Object.keys(bf),...Object.keys(af)]);let changed=0;for(const k of keys){const a=valueText(bf[k]),b=valueText(af[k]);if(a!==b){addState(changesRoot,k,a+'  →  '+b,'changed');changed++}}if(!changed)changesRoot.innerHTML='<div class="empty">No callee receiver fields changed</div>';if(e.__entrySnapshot?.arguments?.length){const h=document.createElement('h3');h.textContent='Arguments';changesRoot.appendChild(h);e.__entrySnapshot.arguments.forEach((v,i)=>addState(changesRoot,'arg'+i,v))}const h=document.createElement('h3');h.textContent='Outcome';changesRoot.appendChild(h);addState(changesRoot,e.thrown?'thrown':'return',e.thrown||e.returnValue);return}document.getElementById('stateTitle').textContent='Invocation state';const before=n.entry?.receiver??e.targetReceiver??e.receiver;const after=n.exit?.receiverAfter??e.receiverAfter;renderSnapshot(beforeRoot,before,'this');renderSnapshot(afterRoot,after,'this');changesRoot.innerHTML='';const bf=fieldsOf(before),af=fieldsOf(after),keys=new Set([...Object.keys(bf),...Object.keys(af)]);let changed=0;for(const k of keys){const a=valueText(bf[k]),b=valueText(af[k]);if(a!==b){addState(changesRoot,k,a+'  →  '+b,'changed');changed++}}if(!changed)changesRoot.innerHTML='<div class="empty">No receiver fields changed</div>';if(n.entry?.arguments?.length){const h=document.createElement('h3');h.textContent='Arguments';changesRoot.appendChild(h);n.entry.arguments.forEach((v,i)=>addState(changesRoot,'arg'+i,v))}if(n.exit){const h=document.createElement('h3');h.textContent='Outcome';changesRoot.appendChild(h);addState(changesRoot,n.exit.thrown?'thrown':'return',n.exit.thrown||n.exit.returnValue)} }
  function renderDetails(){const e=events[selectedEvent];document.getElementById('scopeLocation').textContent=(e.sourceFile||simpleName(e)+'.java')+(e.line?':'+e.line:'');document.getElementById('kind').textContent=e.event==='callsite'?'Call':e.event==='enter'?'Entry':e.event==='exit'?(e.thrown?'Exception':'Exit'):e.event==='resume'?'Resume':'Line';document.getElementById('method').textContent=e.event==='callsite'?'Calls '+simpleName({className:e.calleeClassName})+'.'+e.calleeMethodName+'()':e.event==='resume'?'Resumes '+simpleName(e)+'.'+e.methodName+'()':simpleName(e)+'.'+e.methodName+'()';document.getElementById('depth').textContent='Depth '+Number(e.depth||0);document.getElementById('thread').textContent='Thread '+String(e.thread||e.threadId||'main');document.getElementById('position').textContent=(selectedEvent+1)+' of '+events.length;document.getElementById('prev').disabled=selectedEvent===0;document.getElementById('next').disabled=selectedEvent===events.length-1;renderCode(e);renderState(e)}
  function setMode(m){mode=m;document.getElementById('treeTab').classList.toggle('active',m==='tree');document.getElementById('timelineTab').classList.toggle('active',m==='timeline');document.getElementById('changesTab').classList.toggle('active',m==='changes');m==='timeline'?renderTimeline():renderTree(m==='changes')}
  const persistUi=()=>vscode.setState({...savedUi,sidebarWidth:parseInt(getComputedStyle(shell).getPropertyValue('--sidebar-width'))||360,stateHeight:parseInt(getComputedStyle(shell).getPropertyValue('--state-height'))||220,stateCollapsed:content.classList.contains('state-collapsed')});function installSplitter(el,axis){el.addEventListener('pointerdown',start=>{start.preventDefault();el.setPointerCapture(start.pointerId);el.classList.add('dragging');const startX=start.clientX,startY=start.clientY,side=parseInt(getComputedStyle(shell).getPropertyValue('--sidebar-width'))||360,state=parseInt(getComputedStyle(shell).getPropertyValue('--state-height'))||220;const move=ev=>{if(axis==='x'){const width=Math.max(260,Math.min(window.innerWidth-420,side+ev.clientX-startX));shell.style.setProperty('--sidebar-width',width+'px')}else{content.classList.remove('state-collapsed');const height=Math.max(110,Math.min(content.clientHeight-160,state-(ev.clientY-startY)));shell.style.setProperty('--state-height',height+'px')}};const up=()=>{el.classList.remove('dragging');el.removeEventListener('pointermove',move);el.removeEventListener('pointerup',up);persistUi()};el.addEventListener('pointermove',move);el.addEventListener('pointerup',up)})}installSplitter(document.getElementById('sideSplitter'),'x');installSplitter(document.getElementById('stateSplitter'),'y');document.getElementById('stateTitle').onclick=()=>{content.classList.toggle('state-collapsed');persistUi()};const move=d=>{selectedEvent=Math.max(0,Math.min(events.length-1,selectedEvent+d));renderDetails();setMode(mode)};document.getElementById('prev').onclick=()=>move(-1);document.getElementById('next').onclick=()=>move(1);document.getElementById('open').onclick=()=>{const e=events[selectedEvent];vscode.postMessage({command:e.line?'openLine':'openMethod',className:e.className,methodName:e.methodName,line:e.line,sourcePath:e.sourcePath})};document.getElementById('treeTab').onclick=()=>setMode('tree');document.getElementById('timelineTab').onclick=()=>setMode('timeline');document.getElementById('changesTab').onclick=()=>setMode('changes');window.addEventListener('keydown',e=>{if(e.key==='j'||e.key==='ArrowDown'){move(1);e.preventDefault()}if(e.key==='k'||e.key==='ArrowUp'){move(-1);e.preventDefault()}if(e.key==='Enter')document.getElementById('open').click()});setMode('tree');renderDetails();
  </script></body></html>`;
}
