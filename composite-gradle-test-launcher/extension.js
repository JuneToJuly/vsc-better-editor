...
function renderFlowReplayHtml(result) {
  ...
  *{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);font-family:var(--vscode-font-family)}button{font:inherit}
  ...
  </style></head><body><div class="app"><div class="topbar"><span class="title">Execution Replay</span><div class="tabs"><button id="sourceTab" class="active">Source</button><button id="callsTab">Call Tree</button><button id="timelineTab">Timeline</button><button id="changesTab">Changes</button></div><span style="flex:1"></span><button class="action" id="openSource">Open source</button></div><section class="execution-graph"><div class="graph-title"><strong>Execution path</strong><span id="graphHint">Select a runtime event to inspect its call path.</span><input id="graphSearch" class="graph-search" type="search" spellcheck="false" placeholder="Search methods or source lines…" aria-label="Search execution path"></div><div class="graph-scroll" id="graphScroll"><div class="graph-track" id="graphTrack"></div></div><div class="graph-tooltip" id="graphTooltip"></div></section><div class="workspace"><aside class="pane"><div class="pane-head"><strong id="leftTitle">Executed files</strong><small id="leftHint">Choose the code you want to investigate.</small></div><div class="scroll" id="leftList"></div></aside><main class="pane"><div class="source-head"><div><b id="fileName">Select a file</b><span id="filePath"></span></div><span id="lineSummary"></span></div><div class="scroll" id="sourceScroll"><div class="code" id="code"></div></div></main><aside class="pane activity-pane"><div class="pane-head"><strong id="activityTitle">What happened here</strong><small id="activityHint">Select an executed line to inspect its runtime activity.</small></div><div class="activity-head"><span class="occ" id="occurrence"></span><div class="occ-nav"><button id="prevOcc">←</button><button id="nextOcc">→</button></div></div><div class="scroll" id="activity"></div></aside></div></div><script>
  const vscode=acquireVsCodeApi();const model=${payload};let mode='source';let fileIndex=0;let selectedLine=0;let occurrenceIndex=0;let selectedEventIndex=-1;
  ...
  function chooseLine(line){selectedLine=Number(line);occurrenceIndex=0;const ev=eventsForLine(model.files[fileIndex],selectedLine);selectedEventIndex=ev[0]?.__index??-1;renderSource();renderActivity();}
  ...
  function setMode(next){mode=next;for(const id of ['source','calls','timeline','changes'])document.getElementById(id+'Tab').classList.toggle('active',id===mode);if(mode==='source'){document.getElementById('leftTitle').textContent='Executed files';document.getElementById('leftHint').textContent='Choose the code you want to investigate.';renderFiles()}else if(mode==='calls'){document.getElementById('leftTitle').textContent='Method calls';document.getElementById('leftHint').textContent='Select a call, then inspect its source and state.';traceEvents(e=>e.event==='enter')}else if(mode==='timeline'){document.getElementById('leftTitle').textContent='Full timeline';document.getElementById('leftHint').textContent='All boundaries in chronological order.';traceEvents()}else{document.getElementById('leftTitle').textContent='State changes';document.getElementById('leftHint').textContent='Only boundaries where receiver state changed or an exception occurred.';traceEvents(e=>{const b=e.callerReceiver||e.__entrySnapshot?.receiver||e.receiver,a=e.callerReceiverAfter||e.receiverAfter;return !!e.thrown||(b&&a&&JSON.stringify(b)!==JSON.stringify(a))})}}
  function render(){setMode(mode);renderSource();renderActivity();renderExecutionGraph()}
  document.getElementById('sourceTab').onclick=()=>setMode('source');document.getElementById('callsTab').onclick=()=>setMode('calls');document.getElementById('timelineTab').onclick=()=>setMode('timeline');document.getElementById('changesTab').onclick=()=>setMode('changes');const graphSearch=document.getElementById('graphSearch');graphSearch.addEventListener('input',renderExecutionGraph);graphSearch.addEventListener('keydown',e=>{if(e.key==='Enter'){const first=document.querySelector('.graph-node.search-match');if(first){first.click();e.preventDefault()}}if(e.key==='Escape'){graphSearch.value='';renderExecutionGraph();graphSearch.blur();e.preventDefault()}});document.getElementById('prevOcc').onclick=()=>{occurrenceIndex=Math.max(0,occurrenceIndex-1);renderActivity()};document.getElementById('nextOcc').onclick=()=>{const n=eventsForLine(model.files[fileIndex],selectedLine).length;occurrenceIndex=Math.min(n-1,occurrenceIndex+1);renderActivity()};document.getElementById('openSource').onclick=()=>{const f=model.files[fileIndex];const e=model.events[selectedEventIndex];vscode.postMessage({command:'openLine',sourcePath:e?.sourcePath||f?.sourcePath,className:e?.className,line:e?.line||selectedLine})};window.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='f'){graphSearch.focus();graphSearch.select();e.preventDefault();return}if(document.activeElement===graphSearch)return;if(e.key==='j'||e.key==='ArrowDown'){document.getElementById('nextOcc').click();e.preventDefault()}if(e.key==='k'||e.key==='ArrowUp'){document.getElementById('prevOcc').click();e.preventDefault()}if(e.key==='Enter')document.getElementById('openSource').click()});if(model.files.length)chooseFile(0);else render();
  </script></body></html>`;
}
