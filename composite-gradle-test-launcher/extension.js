...
function renderFlowReplayHtml(result) {
  ...
  *{box-sizing:border-box}html,body{height:100%;overflow:hidden}body{margin:0;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);font-family:var(--vscode-font-family)}button{font:inherit}
  ...
  const valueText=v=>{if(v===undefined)return 'not available';if(v===null)return 'null';if(typeof v==='string'||typeof v==='number'||typeof v==='boolean')return String(v);if(v.display!==undefined)return String(v.display);if(v.value!==undefined&&typeof v.value!=='object')return String(v.value);if(v.summary!==undefined)return String(v.summary);if(Array.isArray(v))return '['+v.map(valueText).join(', ')+']';try{return JSON.stringify(v)}catch{return String(v)}};
  const fieldsOf=s=>{if(!s||typeof s!=='object')return {};if(s.fields&&typeof s.fields==='object'&&!Array.isArray(s.fields))return s.fields;const out={};for(const[k,v]of Object.entries(s)){if(!['type','className','display','identity','id','value','summary'].includes(k))out[k]=v}return out};
  function renderValue(root,value,depth){if(value===undefined||value===null||typeof value!=='object'){const span=document.createElement('span');span.textContent=valueText(value);root.appendChild(span);return}const fields=fieldsOf(value);const items=Array.isArray(value.items)?value.items:null;const entries=Array.isArray(value.entries)?value.entries:null;const hasChildren=Object.keys(fields).length||items?.length||entries?.length;if(!hasChildren||depth>=5){const span=document.createElement('span');span.textContent=valueText(value);root.appendChild(span);return}const details=document.createElement('details');details.className='value-tree';if(depth<1)details.open=true;const summary=document.createElement('summary');summary.textContent=(value.type?String(value.type).split('.').pop()+' ':'')+(value.value&&typeof value.value!=='object'?String(value.value):'')+(value.size!==undefined?' · '+value.size+' items':'');details.appendChild(summary);for(const[k,v]of Object.entries(fields))kv(details,k,v,'',depth+1);if(items)items.forEach((v,i)=>kv(details,'['+i+']',v,'',depth+1));if(entries)entries.forEach((v,i)=>kv(details,'['+i+']',v,'',depth+1));root.appendChild(details)}
  function kv(root,key,value,cls,depth=0){const row=document.createElement('div');row.className='kv '+(cls||'');const keySpan=document.createElement('span');keySpan.className='key';keySpan.textContent=key;const valueSpan=document.createElement('span');row.appendChild(keySpan);row.appendChild(valueSpan);renderValue(valueSpan,value,depth);root.appendChild(row)}
  function snapshot(root,title,s){const h=document.createElement('h3');h.textContent=title;root.appendChild(h);if(!s){root.insertAdjacentHTML('beforeend','<div class="empty">Not available</div>');return}kv(root,'this',s);for(const[k,v]of Object.entries(fieldsOf(s)))kv(root,k,v)}
  function diff(root,before,after){const bf=fieldsOf(before),af=fieldsOf(after),keys=new Set([...Object.keys(bf),...Object.keys(af)]);let n=0;for(const k of keys){const a=valueText(bf[k]),b=valueText(af[k]);if(a!==b){kv(root,k,a+' → '+b,'changed');n++}}if(!n)root.insertAdjacentHTML('beforeend','<div class="empty">No receiver fields changed.</div>')}
  ...
  function replayCallId(e){return String(e?.callId??e?.call??e?.invocationId??'')}
  function replayEntryFor(e){const id=replayCallId(e);if(!id)return null;for(let i=e.__index;i>=0;i--){const x=model.events[i];if(x.event==='enter'&&replayCallId(x)===id)return x}return null}
  ...
  function renderReplayDetail(){const root=document.getElementById('activity');root.innerHTML='';const e=replayEvents[replayPosition];document.getElementById('activityTitle').textContent=e?'Replay step '+(replayPosition+1):'Replay';document.getElementById('activityHint').textContent=e?(eventKind(e)+' · sequence '+String(e.sequence??e.__index)+' · call '+(replayCallId(e)||'n/a')):'No ordered line events were captured.';document.getElementById('occurrence').textContent=e?simple(e)+'.'+(e.methodName||'')+'()':'';document.getElementById('prevOcc').disabled=replayPosition<=0;document.getElementById('nextOcc').disabled=replayPosition>=replayEvents.length-1;if(!e){root.innerHTML='<div class="empty">Run Code Flow with ordered line recording enabled.</div>';return}const context=document.createElement('div');context.className='detail replay-current';kv(context,'event',eventKind(e));kv(context,'sequence',e.sequence??e.__index);kv(context,'source',(e.sourceFile||'')+(e.line?':'+e.line:''));kv(context,'thread',e.thread||e.threadId||'main');kv(context,'call id',replayCallId(e)||'not available');kv(context,'depth',e.depth||0);const code=replaySourceText(e);if(code)kv(context,'code',code);const calls=replayCallsFromLine(e);if(calls.length){const ch=document.createElement('h3');ch.textContent=calls.length===1?'Call from this line':'Calls from this line';context.appendChild(ch);for(const call of calls){kv(context,'calls',simple(call.calleeClassName?{className:call.calleeClassName}:call)+'.'+(call.calleeMethodName||call.methodName||'')+'()');if(call.callerReceiver)snapshot(context,'Caller before call',call.callerReceiver);if(call.arguments?.length){call.arguments.forEach((v,i)=>kv(context,'arg'+i,v))}if(call.thrown)kv(context,'throws',call.thrown);else if(call.returnValue!==undefined)kv(context,'returns',call.returnValue);if(call.callerReceiverAfter){snapshot(context,'Caller after return',call.callerReceiverAfter);diff(context,call.callerReceiver,call.callerReceiverAfter)}}}if(e.frameReceiver){snapshot(context,'State entering this line',e.frameReceiver)}if(e.frameLocals&&Object.keys(e.frameLocals).length){const lh=document.createElement('h3');lh.textContent='Visible locals';context.appendChild(lh);for(const[k,v]of Object.entries(e.frameLocals))kv(context,k,v)}const previousSameCall=replayEvents.slice(0,replayPosition).reverse().find(p=>replayCallId(p)===replayCallId(e));if(previousSameCall&&(previousSameCall.frameReceiver||previousSameCall.frameLocals)){const dh=document.createElement('h3');dh.textContent='Changes since previous line';context.appendChild(dh);let changed=0;if(previousSameCall.frameReceiver&&e.frameReceiver){const before=fieldsOf(previousSameCall.frameReceiver),after=fieldsOf(e.frameReceiver);for(const k of new Set([...Object.keys(before),...Object.keys(after)])){const a=valueText(before[k]),b=valueText(after[k]);if(a!==b){kv(context,'this.'+k,a+' → '+b,'changed');changed++}}}const bl=previousSameCall.frameLocals||{},al=e.frameLocals||{};for(const k of new Set([...Object.keys(bl),...Object.keys(al)])){const a=valueText(bl[k]),b=valueText(al[k]);if(a!==b){kv(context,k,a+' → '+b,'changed');changed++}}if(!changed)context.insertAdjacentHTML('beforeend','<div class="empty">No captured state changed since the previous line in this invocation.</div>')}const h=document.createElement('h3');h.textContent='Active call stack';context.appendChild(h);const stack=document.createElement('div');stack.className='replay-callstack';const path=callPathAt(e.__index);for(const frame of path){const chip=document.createElement('span');chip.className='replay-frame';chip.textContent=simple(frame)+'.'+frame.methodName+'()';stack.appendChild(chip)}if(!path.length)stack.innerHTML='<span class="empty">No active method frame found.</span>';context.appendChild(stack);const entry=replayEntryFor(e),exit=replayExitFor(e);if(entry){snapshot(context,'Method receiver at entry',entry.receiver);if(entry.arguments?.length){const ah=document.createElement('h3');ah.textContent='Method arguments';context.appendChild(ah);entry.arguments.forEach((v,i)=>kv(context,'arg'+i,v))}}if(exit){const oh=document.createElement('h3');oh.textContent='Recorded method outcome';context.appendChild(oh);kv(context,exit.thrown?'thrown':'return',exit.thrown||exit.returnValue);if(exit.receiverAfter){snapshot(context,'Receiver at method exit',exit.receiverAfter);const dh=document.createElement('h3');dh.textContent='Changes during method';context.appendChild(dh);diff(context,entry?.receiver,exit.receiverAfter)}}root.appendChild(context)}
